...
**Claude Code 可用，直连智谱 GLM anthropic 兼容端点，不需要官方 Anthropic 账号。**

```bash
# 一次性测试（实测通过）
claude --settings /opt/data/claude-glm-settings.json -p "只回复两个字：能用" --max-turns 3
```

- **settings 文件**: `/opt/data/claude-glm-settings.json`（含 env：ANTHROPIC_API_KEY / ANTHROPIC_BASE_URL=https://open.bigmodel.cn/api/anthropic / ANTHROPIC_MODEL=glm-5.2，已 chmod 600）
- **上游**: 智谱 GLM-Coding-Plan，**模型必须用 glm-5.2**（Coding Plan 套餐模型）
- **不要用 glm-4.6/glm-4.5 等旧模型名**：智谱对它们报 `1113 余额不足或无可用资源包`，即使 key 有余额——这是模型名错误不是余额问题，别被误导
- **测 key 用 curl**：`curl https://open.bigmodel.cn/api/anthropic/v1/messages -H "x-api-key: $KEY" -H "anthropic-version: 2023-06-01" -d '{"model":"glm-5.2","max_tokens":20,"messages":[{"role":"user","content":"hi"}]}'`，glm-5.2 返回 200 即 key 正常
- **不要走 8648 代理**: Hermes Web UI 的 claude-code-proxy（launch.sh 那套，`hwui_` token）已失效，会 502/Unauthorized。直接 --settings 直连才是正道
- 通用 GLM key（.env 的 GLM_API_KEY）余额可能不足（code 1113），Coding Plan key 有余额——分别验证

## 本环境实测配置（CSDN AI Coding Plan，2026-07-31 ceo 验证通过）

**Claude Code 可用，直连 CSDN Coding Plan OpenAI/Messages 兼容端点。**

```bash
# 一次性测试（实测通过）
claude --settings /opt/data/claude-csdn-settings.json -p "说一个字：好" --max-turns 1
```

- **settings 文件**: `/opt/data/claude-csdn-settings.json`（env：ANTHROPIC_API_KEY / ANTHROPIC_BASE_URL=https://ai.csdn.net/api/model / ANTHROPIC_MODEL=glm_for_coding，chmod 600）
- **关键坑（路径）**: Claude Code 会在 base_url 后自动拼 `/v1/messages`。CSDN 的 messages 端点是 `/api/model/v1/messages`，所以 base_url 必须写 `https://ai.csdn.net/api/model`（**去掉末尾 /v1**），拼出来才正确。写 `.../api/model/v1` 会 461/401
- **关键坑（模型名）**: model 必须填 `glm_for_coding`（底层 glm-5.2，上限 200k 上下文）。**勿填 glm-5.1/glm-5.2，否则按普通模型计费，不扣套餐额度**
- 鉴权：`Authorization: Bearer <KEY>`（Coding Plan 密钥，通用密钥不走套餐）
- 测 key：`curl https://ai.csdn.net/api/model/v1/messages -H "Authorization: Bearer $KEY" -H "anthropic-version: 2023-06-01" -d '{"model":"glm_for_coding","max_tokens":20,"messages":[{"role":"user","content":"hi"}]}'`，200 即正常
- 智谱 vs CSDN 两个套餐可并存：`--settings` 指向哪个文件用哪个
- **CSDN 端点能用但不稳定**：2026-07-31 实测，同一 key 同一端点，16:13 正常、17:06 超时、17:10 又恢复正常。时好时坏，偶尔超时无响应（HTTP 000）。智谱端点稳定，重要任务优先用智谱，CSDN 作为备用
