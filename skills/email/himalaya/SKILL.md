...
- **v2 语法（2026-08-13 实测，本环境 himalaya 2.x）**：`template` 子命令已删除。发信用 `himalaya -a <account> message compose --from <addr> --to <addr> --subject <txt> --body <txt> --send`（成功输出 `Message successfully sent`）。列信用 `himalaya -a <account> envelope list`（`-p 1` 翻页）。账户选择是**全局参数** `-a/--account <NAME>`，放在子命令前。`himalaya smtp send` 是低级 raw SMTP（-f/--mail-from -t/--rcpt-to，stdin 传 RFC5322），普通发信不用它。
